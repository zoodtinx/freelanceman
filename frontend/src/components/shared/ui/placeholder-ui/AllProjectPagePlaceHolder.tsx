...
const NoProjectPlaceholder: React.FC<{ addFn: () => void }> = ({ addFn }) => {
   const PlaceholderBox = () => (
      <div className="border border-secondary border-dashed rounded-container max-w-[400px] h-[205px]"></div>
   );

   const placeholders = [...Array(28)].map(() => <PlaceholderBox />);

   return (
      <div className="grid-cols-[repeat(auto-fit,minmax(260px,1fr))] gap-3 w-full h-full overflow-hidden relative">
