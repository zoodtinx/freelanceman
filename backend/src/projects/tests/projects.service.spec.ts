...
describe('ProjectsService', () => {
  ...
  describe('create', () => {
    it('creates a project', async () => {
      ...
      const result = await service.create('uid', { title: 'New Project' });
      expect(result.id).toBe('1');
    });

    it('throws BadRequest on duplicate', async () => {
      prisma.project.create.mockRejectedValue(
        new Prisma.PrismaClientKnownRequestError('msg', 'P2002', 'project'),
      );
      await expect(
  ...
  describe('update', () => {
    it('updates a project', async () => {
      prisma.project.update.mockResolvedValue({ id: '1', title: 'Updated' });
    ...
    it('throws BadRequest if project not found', async () => {
      prisma.project.update.mockRejectedValue(
        new Prisma.PrismaClientKnownRequestError('msg', 'P2025', 'project'),
      );
      await expect(
