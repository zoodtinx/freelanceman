...
describe('SalesDocumentItemsService', () => {
  ...
  describe('create', () => {
    it('creates a sales document item', async () => {
      ...
      const result = await service.create('doc-1', { description: 'Item A', quantity: 1, unitPrice: 100 });
      expect(result.id).toBe('1');
    });

    it('throws BadRequest on duplicate', async () => {
      prisma.salesDocumentItem.create.mockRejectedValue(
        new Prisma.PrismaClientKnownRequestError('msg', 'P2002', 'salesDocumentItem'),
      );
      await expect(
        service.create('doc-1', { description: 'Item A', quantity: 1, unitPrice: 100 }),
    ...
    it('throws InternalServerError on general error', async () => {
      ...
      await expect(
        service.create('doc-1', { description: '', quantity: 1, unitPrice: 0 }),
  ...
  describe('findOne', () => {
    ...
    it('throws NotFound if item not found', async () => {
      ...
      await expect(service.findOne('doc-1', 'x')).rejects.toThrow(NotFoundException);
    });
  ...
  describe('update', () => {
    it('updates and returns item', async () => {
      prisma.salesDocumentItem.update.mockResolvedValue({ id: '1', description: 'Updated' });
      const result = await service.update('doc-1', '1', { description: 'Updated' });
      expect(result.description).toBe('Updated');
    });

    it('throws BadRequest if item not found (P2025)', async () => {
      prisma.salesDocumentItem.update.mockRejectedValue(
        new Prisma.PrismaClientKnownRequestError('msg', 'P2025', 'salesDocumentItem'),
      );
      await expect(
