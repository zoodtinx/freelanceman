...
jest.mock('src/sales-documents/helpers/pdfUtils', () => ({
...
describe('SalesDocumentsService', () => {
  ...
  describe('create', () => {
    ...
    it('throws BadRequest on duplicate', async () => {
      prisma.salesDocument.create.mockRejectedValue(
        new Prisma.PrismaClientKnownRequestError('msg', 'P2002', 'salesDocument'),
      );
      await expect(
        service.create('uid', { title: '', issuedAt: new Date().toISOString() } as any),
  ...
  describe('update', () => {
    it('updates document', async () => {
      ...
      const result = await service.update('uid', '1', { title: 'Updated' });
      expect(result.id).toBe('1');
    });

    it('throws BadRequest if not found', async () => {
      prisma.salesDocument.update.mockRejectedValue(
        new Prisma.PrismaClientKnownRequestError('x', 'P2025', 'salesDocument'),
      );
      await expect(
